...
const calendar = new Calendar('#calendar', {
    ...
    dataSource:  function(year){
        ...
        return fetch(`https://api.github.com/search/code?q=${year}repo:JiangJY-713/AL_index`)
      .then(result => result.json())
      .then(result => {
        if (result.items) {
        return result.items.map(unpackEntry)
    }
    return [];
    });
    },
...
function unpackEntry(r){
    url = "https://raw.githubusercontent.com/JiangJY-713/AL_Index/main/data/"+r.name;
    return fetch(url).then(entry_info=>entry_info.json())
    .then(function(entry_info){
